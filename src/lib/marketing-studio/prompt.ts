...
import { buildAvatarDescriptor, buildAvatarPrompt, buildTargetCountryPrompt } from './targeting';
...
const SYS = `你是顶尖的产品广告分镜师。把【产品 + 人物 + 玩法 + 开场钩子 + 场景 + 目标国家 + 台词语言 + 视频比例】转成一条真人广告的结构化分镜 JSON,供 Seedance 2.0 视频模型逐镜生成、再拼接成一条完整广告。只输出 JSON,不要解释、不要 markdown 代码块。schema:
{
  "title": "短片名(与产品描述同语种)",
  "ratio": "严格使用用户指定视频比例",
...
export function buildFallbackMarketingPlan(input: PlanInput): MarketingPlan {
  ...
  const shots = Array.from({ length: n }, (_, i) => ({
    ...
    prompt: `A person presents the product (${input.product}); ${targetCountryPrompt ? `${targetCountryPrompt} ` : ''}${
